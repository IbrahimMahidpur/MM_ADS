...
def _quality_gate_node(state: dict) -> dict:
    ...
    errors = state.get("errors", [])
    completed_step = state.get("current_step", 1)
    step_errors = [e for e in errors if f"Step {completed_step}:" in e]
    last_files = state.get("_last_files_created", [])
    if step_errors and not last_files and not state.get("_last_success", True):
        gate_passed = False
    ...
    # If gate failed and we have exhausted retries, advance the step to avoid infinite loop
    if not gate_passed and state.get("retry_count", 0) >= MAX_RETRIES:
        # current_step was already incremented by executor; we keep it as is (points to next step)
        result["retry_count"] = 0  # reset retry count for subsequent tasks
        # Ensure current_step is present (no change needed) but include to be explicit
        result["current_step"] = state.get("current_step", 0)
    return result
...
def _decide_after_gate(state: dict) -> str:
    ...
    # 2. Failed gate but retries exhausted — skip and advance
    if not gate_passed and retry_count >= MAX_RETRIES:
        # Skip this step and let executor move to the next one
...
def _reflection_node(state: dict) -> dict:
    ...
    if incoming_retry_count >= MAX_RETRIES:
        # Retries exhausted - do NOT step back. Keep current_step (which is already pointing to next step)
        # and reset retry_count for the next task
        new_step = current_step
        retry_count = 0
        tasks = state.get("analysis_tasks", [])  # No new guidance needed since we are skipping
    else:
